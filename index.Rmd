...
Above I concluded that the data of interests is still highly multidimensional after performing the PCA and I think its even more evident after running the biplot which seems to be quite messy. However, we can interpret few thing from the plot. If I concentrate to the loading vectors (i.e. the variables), the orange arrows and their closeness to other arrows implies the correlations between the variables. The closer the more correlated with each other. For example the interest toward PC and Cars seem to be higly and positive correlated. Another phenomenon is that the orthogonal arrows suggest negative correlation between the variables. For example interest toward Cars and rPoetry (Poetry reading) share a negative correlation (r = -0.31)
...
## Kaiser criterion and Scree test to retain principal components
