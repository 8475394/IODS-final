...
Considering the upcoming Principal Component Analysis, it is also important to perceive the variables with much lower correlations. For example, it seems that sLeisure and especially the Pets are far away from any centrum.  
...
From the upper right corner the PC and Cars definitely belong to the same dimension. Their arrows are long and they are almost on top of each other suggesting high correlation between them. The Internet indicates to the same direction with a lower contribution, thus it is a question mark for me. Economy and sLeisure (Sport and leisure activities) may point to the same direction but with much lower contribution. Regarding this the latter variables may not include to this dimension. But its good to remember that biplot is just a 2-dimensional presentation suggesting that Economy and sLeisure may not include to second dimension.

The second group contains at least the Physics and SciTec (Science and Technology) with relative high contribution also to second dimension within the biplot presentation.  
...
**WRITE HERE THE INTERPRETATION OF BIPLOT, CHECK THE TEXT ABOVE THE BASIC BIPLOT** Give some explanations of testing codes, etc
...
biplot3 <- fviz_pca_var(PCA_interests, col.var="contrib") + scale_color_gradient2(low="white", mid="blue", high="red", midpoint=0.7) + theme_minimal()
...
fviz_pca_contrib(PCA_interests, choice = "var", axes = 1)

# Contributions of variables on PC1
fviz_pca_contrib(PCA_interests, choice = "var", axes = 2)

# Contributions of variables on PC1
fviz_pca_contrib(PCA_interests, choice = "var", axes = 3)
...
interests_matrix <- data.matrix(interests)

interest_unrot <- principal(r = interests_matrix, nfactors = 8, rotate = "none")
interest_unrot
```
