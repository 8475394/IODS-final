...
# Relationships between different interests
...
## One way to present all the correlations

&nbsp;

Actually, I found a pretty nice solution to present all the correlations of young people's interests, adopted [here](http://geog.uoregon.edu/GeogR/topics/pca.html). 
...
## Performing the PCA and presenting the first biplot


&nbsp;

There are two slightly different methods to perform the PCA: the Eigenvalue Decomposition (princomp) and the Singular Value Decomposition (prcomp). Usually the latter is preferred for its numerical accuracy [STHDA, prcomp vs. princomp](http://www.sthda.com/english/wiki/principal-component-analysis-in-r-prcomp-vs-princomp-r-software-and-data-mining). Thus the Singular Value Decomposition method is used here.
...
biplot(pca_interests, cex = c(0.8, 1), col = c("dimgrey", "darkorange"))
```
...
There are two popular criteria to evaluate the proper amount of principal components. These are the [Kaiser criterion and Cattell's Scree test](http://www.uta.edu/faculty/sawasthi/Statistics/stfacan.html). They both use the eigenvalues of principal conponents to propose the best amount of dimensions. 
...
**Let's take a look at the eigevalues first.** The table below consists also the variances which I interpreted earlier, thus I'm going to concentrate the eigenvalues. Note that I retained the values of first 12 dimension. Regarding the Kaiser criterion, i.e. eigenvalue > 1, it seems that 10 of the the printed dimensions could be meaningful. Let's take a look at the Scree test, if the results will be similar.
...
kable(round(eig.val[1:12,], digits = 3), align="c", caption = 'Eigenvalues and variances')
```


&nbsp;

<p class="text-warning">SCREE PLOT OF EIGENVALUES</p>

**The Scree test** method returns a plot of the variances (y-axis) associated with the principal components (x-axis). The Scree plot can be useful to decide how many component to retain for further analysis.

I retained the above suggested 10 dimensions for the purpose of the Scree plot below. But what does it imply? We can perceive that there are no obvious bendings to help us to make the decision. However, after seventh dimension the line becomes more steady. Thus I will propose that at least the last 3 dimensions could be irrelevant.
...
<p class="text-warning">SCREE PLOT OF EXPLAINED VARIANCE</p>

After the eigenvalues we can evaluate the dimensions to retain also by means of the explained variance. According to [sthda guidelines](http://www.sthda.com/english/wiki/principal-component-analysis-in-r-prcomp-vs-princomp-r-software-and-data-mining#use-the-r-function-prcomp-for-pca) a good dimension reduction is achieved when the first few PCs account for a large proportion of the variablity (80-90%).
...
The rotation measure in prcomp() provides the principal component loadings. Each column of rotation matrix contains the principal component loading vector. Accordin to this [stackexchange conversation](http://stats.stackexchange.com/questions/59213/how-to-compute-varimax-rotated-principal-components-in-r), the term "rotation" is slightly misleading concept, since it indicates more to Factor Analysis. The rotation (in Factor Analysis) is usually applied to loadings, not to eigenvectors of the covariance matrix, which is used to provide the loadings of prcomp(), as far as I know. 

&nbsp;

<p class="text-warning">TABLE OF LOADINGS</p>

Underneath we can see the table of principal component loadings. I printed the loadings of 10 dimension, since I couldn't make any strict decisions by means of the dimension retaining process. It seems that we have a lot of "noise" since most of the loadings are quite. Additionally, there is no clear structure or distinction where the variables load the most. For example the History loads to both 4th and 6th components. Politics loads to 3th and 5th dimensions. Some of the variables load more strongly to one dimension, e.g. Physics (to 2nd comp), PC (to 2nd comp), Economy (to 3rd comp), Biology and Chemistry (both to 3rd comp), and few others.  
...
<p class="text-warning">ENLARGEMENT OF THE BASIC BIPLOT</p>

Now the figure is larger and we can see for example the coordinate system. One feature of the biplot what I didn't mention earlier is that the lenght of the arrows measures the contribution of the variables by means of their loadings. According to this information we can notice that there is at least 3 different group with relative high contribution. 

**From the upper right corner** the PC and Cars definitely belong to the same dimension. Their arrows are long and they are almost on top of each other suggesting high correlation between them. The Internet indicates to the same direction with a lower contribution. Economy and sLeisure (Sport and leisure activities) may point to the same direction but with much lower contribution. Regarding this, the latter variables may not include to this or any dimension. 

**The second group** contains at least the Physics and SciTec (Science and Technology) with relative high contribution also to second dimension within the biplot presentation. The Mathematics, sAdrenaline and sCompetitive may include to the same dimension but with lower contribution.

**Then there are some "middle" variables** which don't share so high groupping features, thus its more difficult to interpret where these variables belong. However, iti starting to look that at least the Pets and Social (Solializing with friends) don't belong to any principal component. 

**From the lower left corner** we find at least one meaningful group including Art Exhibitions, Theatre and Poetry Reading. The Dancing and Poetry Reading may also include to this dimension.  

I think that we can find at least one dimension including Medicine, Biology, Psychology and maybe Religion, Instruments and Outdoor Activities.

To conlcude thus far I think its clear that the variables with the highest contribution share also some similar features. For example Art, Theatre and Poetry Reading, and even Dancing and Poetry writing are all representing interest relating to arts. The phenomenon is equivalent with Medicine, Biology and Psychology, because they all indicate interest toward people and their well-being. Furthermore the SciTec, Physics and Mathematics depict the features of mathematical and technical issues. And finally the PC and Cars indicate both to different kind of technical features comparing to former group. 
...
<p class="text-warning">BIPLOT WITH FACTOMINER</p>


For the more visual purposes I attached the FactoMiner and factoextra packages. FactoMiner has its own function for Principal Component Analysis, "PCA()". Note that I used the factoextra earlier to visualize the Scree plots but now it seems that the biplot visualization works better with its own function. For this reason I performed the Principal Component Analysis again. I wanted to understand some of the mathematics behind the package and therefore I have some additional "testing codes" inside the chunk below. 

For some reason the method turns the arrows the other way round but it doesn't change the results. From this biplot you can observe basically the same interpretations as I wrote above but with additional coloring effects. Note that I have set the "midpoint" relative low to present the variation within this data. Here it is more evident that the variables with very light colour do not have high contribution to any dimension.  

&nbsp;


```{r, fig.height=7, fig.width=8, message=FALSE, warning=FALSE}
library(FactoMineR); library(factoextra); library(knitr)

# http://www.sthda.com/english/wiki/principal-component-analysis-how-to-reveal-the-most-important-variables-in-your-data-r-software-and-data-mining#at_pco=smlwn-1.0&at_si=58bbc17c84dcd355&at_ab=per-2&at_pos=0&at_tot=1
...
# The squared loadings, i.e. variance^2
...
## Conclusions of the first PCA part

&nbsp;

It is evident that the data of interests contains at least 3 clear dimensions while their exact content of variables is unclear. Secondly, it is not so clear how many other dimension, not to mention their content, the data may comprise regarding the available tools. 

It is also apparent that all of these interests are not equally important and it seems that some of them aren't popular at all and some of them just don't fit to any dimension. This means that I need some new tools to wrangle the data to better suit my purposes. 

Note, that I didn't want to create the dimensions based on incomplete or just directional results. Thus, let's move on to the next part of Principal Component Analysis.



&nbsp;

# PCA Analysis, Round 2

&nbsp;

Here I adopt an additional approach to better identify the variables which have very low contribution in terms of dimension reduction considering the Young People's Interests. I loaded the [psych package](https://cran.r-project.org/web/packages/psych/) and I'm going to use the "principal()" function to get more statistical results at once. The idea came from [this site](http://geog.uoregon.edu/GeogR/topics/pca.html) which is the same page from where I adpoted the graphic of correlation network. I'm also going to benefit [these instructions](http://www.ic.unicamp.br/~wainer/cursos/1s2013/ml/Lecture18_PCA.pdf) to perform the PCA with this new function.

Note also that I'm going to use simultaneously this new and previous pca functions to draw graphics since they all don't fit with each other.

First I introduce briefly the methods behind variable extraction. After that the purpose is to show the new structure and form the factor scores with respect to actually utilize the new dimensions for example for some form of regression analysis.

&nbsp;

## Performing PCA with principal()

&nbsp;

Below you can see many statistical results of the PCA which are basically same than earlier. Regarding to extract some variables and retain proper amount of dimensions, I had to work simultaneously with earlier information and two different estimator and adjust the number of dimensions and drop variables from the data one by one. Since its not convenient to print like 10 long tables, I try to explain shortly what I did. 

Thus, I followed the values of communalities (h^2) and proportion of variance of each component. My tresholds were h^2 < 0.30 to drop variables and around 5% proportion of variance to retain dimensions. First I performed PCA with different numbers of component and located variables with especially low communalities. First variables to drop were Pets and sLeisure (Sport and leisure activities) and after that Social (Socializing with friends), Gardening and Religion. Note that the combination of values changes every time when you drop even one variable.

While dropping the variables I followed the variance proportion of each dimension. Finally, after dropping the above mentioned variables I had to choose 6 dimensions instead of 7. This was because the variance proportion was too low in seventh dimension.


&nbsp;

<p class="text-warning">PCA RESULTS WITH 8 DIMENSIONS</p>
...
interest_unrot <- principal(r = std_interests, nfactors = 8, rotate = "none")
interest_unrot
```
...
<p class="text-warning">DROPPING THE VARIABLES</p>

From the chunk you can see that after dropping the variables I also scaled the data. I also printed the column names to verify the content. 


```{r, message=FALSE, warning=FALSE}
library(dplyr); library(clusterSim)

# Printing the exact names of variables
noprinting <- names(interests) # for my own use

# Extracting the variables, which h^2 < 0.3 or otherwise have low contribution
hobby <- dplyr::select(interests, -Pets, - sLeisure, -Social, -Gardening, -Religion)
names(hobby)

# Scaling the variables
std_hobby <- data.Normalization(hobby, type = "n1", normalization = "columns")
...
<p class="text-warning">PCA WITH THE NEW DATA</p>

Here I performed the PCA with the new data with 6 dimensions. Now all the communalities suit well even though there are 2 variables close but not lower than 0.30, i.e. Geography and Psychology. The "SS loadings" are the eigenvalues and all of them are more than 1 (in terms of the Kaiser criterion). Additionally the proportion of variance is 5% or more within each dimension. But we have at least one weakness and that is the cumulative variance which is only around 50%. This means, that if we keep this structure we will lose some information considering the data. Thus it seems that when one value or result appears to be correct the other value is missing something.  
...
```{r, message=FALSE, warning=FALSE}
library(psych)
pca1_hobby <- prcomp(std_hobby) # Used only for drawing graphs

# PCA of 6 dimensions with new data
hobby_unrot <- principal(r = std_hobby, nfactors = 6, rotate = "none")
hobby_unrot
...
<p class="text-warning">BIPLOT OF NEW DATA</p>

As you can perceive the biplot seems to be quite similar than earlier.
...
biplot3 <- fviz_pca_var(pca1_hobby, col.var="cos2") + scale_color_gradient2(low="white", mid="blue", high="red", midpoint=0.2) + theme_minimal()
...
contr1 <- fviz_pca_contrib(pca1_hobby, choice = "var", axes = 1, top = 8)

# Contributions of variables on PC2
contr2 <- fviz_pca_contrib(pca1_hobby, choice = "var", axes = 2, top = 8)

# Contributions of variables on PC3
contr3 <- fviz_pca_contrib(pca1_hobby, choice = "var", axes = 3, top = 8)

# Contributions of variables on PC4
contr4 <- fviz_pca_contrib(pca1_hobby, choice = "var", axes = 4, top = 8)

# Contributions of variables on PC5
contr5 <- fviz_pca_contrib(pca1_hobby, choice = "var", axes = 5, top = 8)

# Contributions of variables on PC6
contr6 <- fviz_pca_contrib(pca1_hobby, choice = "var", axes = 6, top = 8)

# Printing all at once
grid.arrange(contr1, contr2, contr3, contr4, contr5, contr6, ncol = 2, nrow =3)
...
library(qgraph)
qg_pca <- qgraph.pca(hobby, factors=6, rotation="varimax", posCol="darkmagenta", negCol="darkgreen")
```
...
```{r, message=FALSE, warning=FALSE}
library(psych)
hobby_rot <- principal(r = std_hobby, nfactors = 6, rotate = "varimax", scores = T)
hobby_rot
```
