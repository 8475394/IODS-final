...
Note that the histograms are showed regarding the original variable list while the structure is organized by proposed groups or components. Note also that gender as well as the ditribution of age are visualized only for the informative purposes.
...
Here my intention is to explore the potential relationships between the different interests and/or hobbies. However, I had this notable problem with the amount of my variables. First, I printed the correlation matrix including all 32 variable but the table grew so large that it was very difficult to read that. Also, it didn't fit to the web page well. Thus, I divided the dataset into the 3 subset to present the correlations at least in some extent. 
...
The first table informs for example that History is related to Politics and Psychology. In my preliminary hypothesis I didn't count the psychology in to the same dimension, thus it is interesting to see what is the final result of Principal Component Analysis. Then you can perceive that Mathematics is quite certainly assiociated to Physics as I thought from the beginning. The same amount of certainty lies between Chemistry and Biology. interestingly, Internet shares a relationship with PC, which I didn't recognize from their histograms. Finally in this table the Economy shares a moderate association with Politics.     
...
# Plotting the correlations betweenv the rest of the ariables along with the significance test
col <- colorRampPalette(c("#BB4444", "#EE9988", "#FFFFFF", "#77AADD", "#4477AA"))
