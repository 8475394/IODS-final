...
Now the figure is larger and we can see for example the coordinate system. One feature of the biplot what I didn't mention earlier is that the lenght of the arrows measures the contribution of the variables by means of their loadings. 

We can evaluate first the 2-dimensional presentation in terms of first and second dimension. It is evident that the Art, Theatre and Poetry reading (rPoetry) load the most to **the first dimension**. Also the Medicine, Biology and Psychology are pointing to the first dimension even though these might represent their own group or dimension regarding their meaning. The Poetry writing (wPoetry) and Dancing are also loading to the first dimension, but I think that the Dancing doesn't share the same features in a "real life". Then there are multiple variables with lower loading pointing to the first dimension, for example Religion and playing instruments.

Then, considering **the second dimension**, the PC, Cars, Physics and SciTec (Science and Technology) are loading the most. Additionally, the Mathematics, sAdrealine (Adrenaline sports) and Internet are pointing to this dimension, even though I think that the Adrenaline Sports doesn't share the meaning with other variables.    

Then if we look at the **different layers** regarding the leght from the centrum of the coordinate system, we can see for example that the Shopping, Celebrities, Socializing with friends and Pets could yield one dimension, even though their overall contribution is lower. 

**Then there are some problematic variables** also in terms with my initial hypotheses. For example, variables considering some kind of sport, i.e. Outdoor activities, sLeisure, sCompetitive, sAdrenaline and maybe Dancing, are pointing to very different directions or contributing with very different amount, even thoug the sCompetitive and sAdrenaline are somewhat near with each other and may share a meaning. But these other "sportish" variables seem to differ, thus here's what I think: All of the variables are not mutually exclusive. For example, Outdoor activities, sLeisure (Sport and leisure activities) and even Dancing may already include to variable of Socialisizing with friend. Accordingly, I think that I'll try to extract at least the sLeisure and Outdoor Activities from the data before testing the second round of PCA.

But before performing the second round, I have to consider **the rest of the puzzling variables**, which seem to be more complicated, including for example the Languages, Gardening and maybe Religion, Instruments and Economy. Especially the languages and Religion are problematic in a way that they can relate to any of these dimensions considering their meaning or they can be totally independent. The Gardening and Instruments can include at least to the artistic or socializing features and Economy may relate to Politics and Law. Thus, I think I'll try extract at least Languages and Religion.  

Before the data wrangling, you can take a look at the biplots and figures of the variable contribution of different dimensions.
...
<p class="text-warning">VARIABLE CONTRIBUTION TO DIFFERENT DIMENSIONS</p>

Here you can examine the varible contribution between different dimension and compare your interpretation with the one I wrote above. Note that the content of last dimensions have no meaningful order or sense. 
...
# PART 2, Testing some assumptions

&nbsp;

## Extracting first set of variables
...
dim(interests)
names(interests) # for my own use
...
```{r, message=FALSE, warning=FALSE}
pca_testdata1 <- prcomp(std_testdata1)

# Printing a summary of PCA
summary(pca_testdata1)
```
...
eig.test1 <- get_eigenvalue(pca_testdata1)
kable(round(eig.val[1:10,], digits = 3), align="c", caption = 'Eigenvalues and variances')
```

&nbsp;

## Extracting second set of variables
...
```{r, warning=FALSE}
pca_testdata2 <- prcomp(std_testdata2)

# Printing a summary of PCA
summary(pca_testdata2)
```
...
biplot4 <- fviz_pca_var(pca_testdata2, col.var="cos2") + scale_color_gradient2(low="white", mid="blue", high="red", midpoint=0.2) + theme_minimal()
...
# Contributions of variables on PC1
test1 <- fviz_pca_contrib(pca_testdata2, choice = "var", axes = 1, top = 10)

# Contributions of variables on PC2
test2 <- fviz_pca_contrib(pca_testdata2, choice = "var", axes = 2, top = 8)

# Contributions of variables on PC3
test3 <- fviz_pca_contrib(pca_testdata2, choice = "var", axes = 3, top = 8)

# Contributions of variables on PC4
test4 <- fviz_pca_contrib(pca_testdata2, choice = "var", axes = 4, top = 8)

# Contributions of variables on PC5
test5 <- fviz_pca_contrib(pca_testdata2, choice = "var", axes = 5, top = 8)

# Contributions of variables on PC6
test6 <- fviz_pca_contrib(pca_testdata2, choice = "var", axes = 6, top = 8)

# Printing all at once
grid.arrange(test1, test2, test3, test4, test5, test6, ncol = 2, nrow =3)
```
...
print.psych(interest_unrot, sort = T)
```
...
interests_rot <- principal(r = testdata2, nfactors = 5, rotate = "varimax", scores = T)
print.psych(interests_rot, sort = T)
```
